...
fun main() {
    val sortedMembersByParty = sortByParty(ParliamentMembersData.members)
    println("Number of members by party:")
    println(sortedMembersByParty.joinToString(", ") { "${it.first}: ${it.second}" })

    val sortedMembersByName = sortByName(ParliamentMembersData.members)
    println("**")
    println("Sorted members by name:")
    sortedMembersByName.forEach { println("${it.firstname} ${it.lastname}") }

    val seatCountByParty = countSeatsByParty(ParliamentMembersData.members)
    println("Number of seats by party:")
    seatCountByParty.forEach { println("${it.key}: ${it.value}") }
}

fun sortByName(members: List<ParliamentMember>): List<ParliamentMember> {
    return members.sortedWith(compareBy({ it.firstname }, { it.lastname }))
}

fun sortByParty(members: List<ParliamentMember>): List<Pair<String, Int>> {
    return members.groupBy { it.party }
        .map { it.key to it.value.size }
        .sortedBy { it.first }
        .sortedByDescending { it.second }
    }

fun countSeatsByParty(members: List<ParliamentMember>): Map<String, Int> {
    return members.groupBy { it.party }
